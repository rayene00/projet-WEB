...
<main class="offres-wrapper">
    <div class="offres-container">
        <!-- FILTRES -->
        <aside class="filtres">
            <form method="GET" action="{{ route('offres') }}" id="filtres-form">
                <!-- Catégories -->
                <div class="filtre-bloc">
                    <div class="filtre-header"><span>Catégories</span></div>
                    @foreach ($categoriesList as $cat)
                        <label class="checkbox-wrapper">
                            <input type="checkbox" name="categories[]" value="{{ $cat }}" {{ in_array($cat, $selectedCategories) ? 'checked' : '' }}>
                            <span>{{ $cat }} ({{ $categoryCounts[$cat] ?? 0 }})</span>
                        </label>
                    @endforeach
                </div>
                ...
                <div class="filtre-bloc">
                    ...
                    <label class="checkbox-wrapper">
                        <input type="checkbox" name="duree[]" value="1-3" {{ in_array('1-3', $selectedDurees) ? 'checked' : '' }}>
                        <span>1 à 3 mois</span>
                    </label>
                    <label class="checkbox-wrapper">
                        <input type="checkbox" name="duree[]" value="3-6" {{ in_array('3-6', $selectedDurees) ? 'checked' : '' }}>
                        <span>3 à 6 mois</span>
                    </label>
                    <label class="checkbox-wrapper">
                        <input type="checkbox" name="duree[]" value="6+" {{ in_array('6+', $selectedDurees) ? 'checked' : '' }}>
                ...
                @foreach ($offres as $offre)
                    ...
                    <div class="offre">
                        ...
                        <div class="offre-details">
                            <h2>{{ $offre->titre }}</h2>
                            <p>{{ $offre->nom_entreprise }} - {{ $offre->ville }}</p>
                            <div class="tags">
